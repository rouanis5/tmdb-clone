...
const NavbarLink = ({ name, url, subLinks }: LinkPropsType) => {
  return (
    <li className="group relative inline-block p-2 capitalize">
      <Link href={url} className="font-semibold text-white space-x-2 group">
        <span>{name}</span>
        <ChevronDown className="inline size-5 transition-transform duration-300 group-hover:-scale-100" />
      </Link>
